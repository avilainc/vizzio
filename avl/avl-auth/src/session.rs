...
//! - Distributed session storage (in-memory + AvilaDB)
...
impl Default for SessionConfig {
    fn default() -> Self {
        Self {
            ...
            absolute_timeout: Duration::hours(24),
...
mod tests {
    ...
    async fn test_session_creation() {
        ...
        assert_eq!(session.device_id, Some("device_123".to_string()));
        assert_eq!(session.scopes.len(), 2);
    }
    ...
    async fn test_concurrent_session_limit() {
        ...
        let user_sessions = manager.list_user_sessions(&user_id).await;
        assert_eq!(user_sessions.len(), 2); // Should only have 2 sessions
    }

    #[tokio::test]
    async fn test_session_expiration() {
        let mut config = SessionConfig::default();
        config.idle_timeout = Duration::seconds(1);
        let manager = SessionManager::new(config);
        ...
        let result = manager.validate_session(&session.id, None, None).await;
