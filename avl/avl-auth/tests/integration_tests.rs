...
async fn test_concurrent_sessions() {
    ...
    let user_id = client.register(email.clone(), password.clone()).await.unwrap();
    ...
    let sessions = client.session_manager().list_user_sessions(&user_id).await;
    assert!(sessions.len() <= 5); // Max concurrent sessions
}
...
async fn test_role_based_access() {
    ...
    // Assign roles
    client.permissions_manager()
        .assign_role(user_id, "admin".to_string())
        .await
        .unwrap();

    let has_role = client.permissions_manager()
...
async fn test_permission_checking() {
    ...
    client.permissions_manager()
        .grant_permission(user_id, "users:read".to_string())
        .await
        .unwrap();

    let has_permission = client.permissions_manager()
        .has_permission(&user_id, "users:read")
        .await
        .unwrap();
...
async fn test_risk_assessment() {
    ...
    let assessment = client.risk_manager()
        .assess_login_risk(
...
async fn test_audit_logging() {
    ...
    client.audit_manager()
        .log_event(
            Some(user_id),
            None,
            "user.login".to_string(),
            "session".to_string(),
            avl_auth::models::AuditResult::Success,
            Some("127.0.0.1".parse().unwrap()),
            None,
    ...
    let logs = client.audit_manager()
        .get_user_logs(&user_id, 10)
        .await
        .unwrap();
...
async fn test_device_trust() {
    ...
    let device_info = avl_auth::models::DeviceInfo {
        device_id: "device_123".to_string(),
        device_name: "Test Device".to_string(),
        device_type: avl_auth::models::DeviceType::Desktop,
    ...
    client.device_trust_manager()
        .register_device(user_id, device_info.clone())
        .await
        .unwrap();

    let is_trusted = client.device_trust_manager()
        .is_device_trusted(&user_id, "device_123")
        .await
        .unwrap();

    assert!(!is_trusted); // New devices are not trusted by default
}
...
async fn test_token_refresh() {
    ...
    // Refresh token
    let new_session = client.refresh_token(&session.refresh_token).await.unwrap();

    assert_ne!(old_access_token, new_session.access_token);
}
...
async fn test_session_cleanup() {
    ...
    // Create multiple sessions
    for i in 0..5 {
        let email = format!("cleanup_{}@example.com", i);
        client.register(email.clone(), "SecureP@ssw0rd123!".to_string()).await.unwrap();
    }

    // Run cleanup
    let removed = client.session_manager().cleanup_expired_sessions().await.unwrap();
    assert!(removed >= 0);
}
