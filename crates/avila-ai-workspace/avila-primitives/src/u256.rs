...
impl U256 {
    ...
    pub const fn from_hex_const(_hex: &str) -> Self {
    ...
    pub const fn mul_u64(&self, rhs: u64) -> Self {
        let mut result = Self::ZERO;
        let (lo, mut carry) = mul_wide(self.limbs[0], rhs);
        result.limbs[0] = lo;

        let (lo, new_carry) = mac(self.limbs[1], rhs, carry, 0);
        result.limbs[1] = lo;
        carry = new_carry;

        let (lo, new_carry) = mac(self.limbs[2], rhs, carry, 0);
        result.limbs[2] = lo;
        carry = new_carry;
