...
fn columnar_storage_example() {
    ...
    let num_sessions: usize = 10000;

    // Column 1: Session IDs (sequential)
    let session_ids: Vec<u32> = (1..=num_sessions as u32).collect();
    ...
    let mut levels: Vec<u32> = Vec::with_capacity(num_sessions);
    for i in 0..num_sessions {
        let level: u32 = match i % 100 {
            0..=50 => 1,
            51..=75 => 10,
            76..=90 => 25,
            _ => 50,
    ...
    let play_times: Vec<u32> = (0..num_sessions)
        .map(|i| 300u32 + ((i % 3600usize) as u32))
        .collect();
