...
pub mod btree {
    ...
    impl<K: Ord + Clone, V: Clone> BPlusTree<K, V> {
        ...
        /// Insert key-value pair with full split handling
        pub fn insert(&mut self, key: K, value: V) -> Option<V> {
            if self.root.is_none() {
                let mut keys = core::array::from_fn(|_| None);
            ...
            let old_value = self.insert_recursive(self.root.as_mut().unwrap(), key.clone(), value);
        ...
        fn insert_recursive(&mut self, node: &mut Node<K, V>, key: K, value: V) -> Option<V> {
            match node {
                Node::Leaf { keys, values, .. } => {
                    // Find position
                    let mut pos = 0;
                    while pos < NODE_SIZE && keys[pos].is_some() {
                        if let Some(ref k) = keys[pos] {
                            match key.cmp(k) {
                                core::cmp::Ordering::Equal => {
                                    // Update existing
                                    return values[pos].replace(value);
                                }
                                core::cmp::Ordering::Less => break,
                                core::cmp::Ordering::Greater => pos += 1,
                            }
                        }
                    }
                ...
                Node::Internal { keys, children } => {
                    ...
                    if let Some(ref mut child) = children[pos] {
        ...
        /// Get value by key with full tree traversal
                ...
                match current.as_ref() {
                    Node::Leaf { keys, values, .. } => {
                        for i in 0..NODE_SIZE {
                            if let Some(ref k) = keys[i] {
                                if k == key {
                                    return values[i].as_ref();
                                }
                            }
                        }
                        return None;
...
pub mod rbtree {
    ...
    impl<K: Ord, V> RBTree<K, V> {
        ...
        fn insert_rec(node: Option<Box<Node<K, V>>>, key: K, value: V) -> Option<Box<Node<K, V>>> {
            ...
            Some(Self::fix_violations(node))
        }

        fn fix_violations(mut node: Box<Node<K, V>>) -> Box<Node<K, V>> {
        ...
        pub fn remove(&mut self, key: &K) -> Option<V> {
            ...
            let (new_root, removed) = Self::remove_rec(self.root.take(), key);
            self.root = new_root;
        ...
        fn remove_rec(node: Option<Box<Node<K, V>>>, key: &K) -> (Option<Box<Node<K, V>>>, Option<V>) {
            let mut node = match node {
            ...
            let removed = match key.cmp(&node.key) {
                core::cmp::Ordering::Less => {
                    let (new_left, val) = Self::remove_rec(node.left.take(), key);
                    node.left = new_left;
                    (Some(node), val)
                }
                core::cmp::Ordering::Greater => {
                    let (new_right, val) = Self::remove_rec(node.right.take(), key);
...
///
/// **Algorithm**: Binary space partitioning for k dimensions
/// **Performance**: O(log N) nearest neighbor in low dimensions
pub mod kdtree {
    use super::*;
    ...
    impl<const K: usize> Point<K> {
        ...
        pub fn distance_squared(&self, other: &Point<K>) -> f64 {
            self.coords
                .iter()
    ...
    struct KDNode<const K: usize, V> {
        point: Point<K>,
        value: V,
        left: Option<Box<KDNode<K, V>>>,
        right: Option<Box<KDNode<K, V>>>,
    }

    pub struct KDTree<const K: usize, V> {
        root: Option<Box<KDNode<K, V>>>,
        size: usize,
    }

    impl<const K: usize, V> KDTree<K, V> {
        pub fn new() -> Self {
            Self {
                root: None,
                size: 0,
        ...
        pub fn insert(&mut self, point: Point<K>, value: V) {
            self.root = Self::insert_rec(self.root.take(), point, value, 0);
            self.size += 1;
        }

        fn insert_rec(
            node: Option<Box<KDNode<K, V>>>,
            point: Point<K>,
            value: V,
            depth: usize,
        ) -> Option<Box<KDNode<K, V>>> {
            let mut node = match node {
                None => {
            ...
            if point.coords[axis] < node.point.coords[axis] {
                node.left = Self::insert_rec(node.left.take(), point, value, depth + 1);
            } else {
                node.right = Self::insert_rec(node.right.take(), point, value, depth + 1);
            }

            Some(node)
        }

        pub fn nearest(&self, target: &Point<K>) -> Option<(&Point<K>, &V)> {
            let mut best: Option<(&Point<K>, &V, f64)> = None;
            Self::nearest_rec(self.root.as_ref()?, target, &mut best, 0);
            best.map(|(p, v, _)| (p, v))
        }
        ...
        fn nearest_rec<'a>(
            node: &'a KDNode<K, V>,
            target: &Point<K>,
            best: &mut Option<(&'a Point<K>, &'a V, f64)>,
            depth: usize,
        ) {
            let dist = node.point.distance_squared(target);
            ...
            if let Some(ref child) = first {
                Self::nearest_rec(child, target, best, depth + 1);
            }

            if diff.powi(2) < best.unwrap().2 {
...
/// QuadTree - 2D spatial partitioning tree
///
/// **Algorithm**: Divide 2D space into 4 quadrants recursively
/// **Performance**: O(log N) average for spatial queries
pub mod quadtree {
    use super::*;
    ...
    struct QuadNode<T> {
        boundary: Rectangle,
        points: DynamicArray<(Point2D, T)>,
        nw: Option<Box<QuadNode<T>>>,
        ne: Option<Box<QuadNode<T>>>,
        sw: Option<Box<QuadNode<T>>>,
        se: Option<Box<QuadNode<T>>>,
    }
    ...
    impl<T> QuadTree<T> {
        ...
        pub fn insert(&mut self, point: Point2D, data: T) -> bool {
            Self::insert_rec(&mut self.root, point, data, self.capacity)
        }

        fn insert_rec(node: &mut QuadNode<T>, point: Point2D, data: T, capacity: usize) -> bool {
            if !node.boundary.contains(&point) {
                return false;
            }

            if node.points.len() < capacity {
            ...
            if Self::insert_rec(node.nw.as_mut().unwrap(), point, data, capacity) {
                return true;
            }
            if Self::insert_rec(node.ne.as_mut().unwrap(), point, data, capacity) {
                return true;
            }
            if Self::insert_rec(node.sw.as_mut().unwrap(), point, data, capacity) {
                return true;
            }
            if Self::insert_rec(node.se.as_mut().unwrap(), point, data, capacity) {
                return true;
            }
        ...
        fn subdivide(node: &mut QuadNode<T>) {
            let x = node.boundary.x;
            let y = node.boundary.y;
            let w = node.boundary.width / 2.0;
            let h = node.boundary.height / 2.0;

            node.nw = Some(Box::new(QuadNode {
                boundary: Rectangle::new(x, y, w, h),
                points: DynamicArray::new(),
                nw: None,
                ne: None,
                sw: None,
            ...
            node.ne = Some(Box::new(QuadNode {
                boundary: Rectangle::new(x + w, y, w, h),
                points: DynamicArray::new(),
                nw: None,
                ne: None,
                sw: None,
            ...
            node.sw = Some(Box::new(QuadNode {
                boundary: Rectangle::new(x, y + h, w, h),
                points: DynamicArray::new(),
                nw: None,
                ne: None,
                sw: None,
            ...
            node.se = Some(Box::new(QuadNode {
                boundary: Rectangle::new(x + w, y + h, w, h),
                points: DynamicArray::new(),
                nw: None,
                ne: None,
                sw: None,
        ...
        pub fn query(&self, range: &Rectangle) -> DynamicArray<&T> {
            let mut found = DynamicArray::new();
            Self::query_rec(&self.root, range, &mut found);
            found
        }

        fn query_rec<'a>(node: &'a QuadNode<T>, range: &Rectangle, found: &mut DynamicArray<&'a T>) {
            ...
            if let Some(ref nw) = node.nw {
                Self::query_rec(nw, range, found);
            }
            if let Some(ref ne) = node.ne {
                Self::query_rec(ne, range, found);
            }
            if let Some(ref sw) = node.sw {
                Self::query_rec(sw, range, found);
            }
            if let Some(ref se) = node.se {
                Self::query_rec(se, range, found);
            }
        }
    }
}
