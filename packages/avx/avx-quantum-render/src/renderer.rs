...
impl QEDRenderer {
    ...
    fn sample_photon_path<R: Rng>(&self, scene: &Scene, u: f64, v: f64, rng: &mut R) -> PhotonPath {
        ...
        while depth < self.config.max_path_depth {
            ...
            if let Some((surface, hit)) = self.ray_intersect(scene, current_pos, current_dir) {
                ...
                current_dir = self.sample_new_direction_with_hit(surface, current_dir, hit.normal, interaction, rng);
                depth += 1;
            } else {
    ...
    fn ray_intersect<'a>(
        ...
        origin: [f64; 3],
        direction: [f64; 3],
    ) -> Option<(&'a Surface, crate::scene::HitRecord)> {
        let mut closest_hit: Option<(&'a Surface, crate::scene::HitRecord)> = None;

        for surface in &scene.surfaces {
            if let Some(hit) = surface.intersect(origin, direction) {
                let update = closest_hit
                    .as_ref()
                    .map(|(_, h)| hit.distance < h.distance)
                    .unwrap_or(true);

                if update {
                    closest_hit = Some((surface, hit));
                }
            }
        }
    ...
    /// Amostra nova direção após interação (usando normal do hit)
    fn sample_new_direction_with_hit<R: Rng>(
        &self,
        surface: &Surface,
        incident: [f64; 3],
        normal: [f64; 3],
        ...
        match interaction {
            InteractionType::Reflection => self.reflect(incident, normal),
            InteractionType::Scattering => self.sample_hemisphere(normal, rng),
            InteractionType::Refraction => self.refract(incident, normal, surface.material.refractive_index(), rng),
            _ => incident,
        }
    }

    /// Amostra nova direção após interação (versão antiga com Surface)
    #[allow(dead_code)]
    fn sample_new_direction<R: Rng>(
        ...
        // Delega para versão com hit normal usando material do surface
        let dummy_normal = match surface.geometry {
            crate::scene::Geometry::Plane { normal, .. } => normal,
            crate::scene::Geometry::Sphere { .. } => {
                // Para esfera, normal será calculado no hit; usar direção como fallback
                incident
            }
            _ => incident,
        };
        self.sample_new_direction_with_hit(surface, incident, dummy_normal, interaction, rng)
    }
