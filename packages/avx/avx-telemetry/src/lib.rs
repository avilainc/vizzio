...
use avila_tracing::{ConsoleLogger, Level};
...
pub fn init_tracing(ctx: &AvxContext) {
    LOGGER_INIT.call_once(|| {
        avila_log::init(ConsoleLogger::new(Level::Info));
    });

    avila_log::info!("🔍 Avx telemetry initialized:");
    avila_log::info!("   Stack: {}", ctx.stack);
    avila_log::info!("   Layer: {}", ctx.layer);
    avila_log::info!("   Env: {}", ctx.env);
    avila_log::info!("   Cluster: {}", ctx.cluster);
    avila_log::info!("   Mesh: {}", ctx.mesh);
}
