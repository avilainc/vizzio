...
mod tests {
    ...
    fn test_statistics_to_json() {
        ...
        let obj = json
            .as_object()
            .expect("Latency statistics JSON should be an object");

        assert_eq!(
            obj.get("count").and_then(|v| v.as_f64()),
            Some(100.0)
        );
        assert_eq!(
            obj.get("mean_ms").and_then(|v| v.as_f64()),
            Some(15.5)
        );
        ...
        let percentiles = obj
            ...
            .expect("percentiles should be an object");

        assert_eq!(
            percentiles.get("p50").and_then(|v| v.as_f64()),
            Some(15.0)
        );
        assert_eq!(
            percentiles.get("p99").and_then(|v| v.as_f64()),
            Some(24.0)
        );
    }
